...
class ImageSkewCorrector:
    ...
    def find_score(self, angle):
        data = inter.rotate(self.img, angle, reshape=False, order=0)
    ...
    def correct_skew(self):
        ...
        data = inter.rotate(self.img, best_angle, reshape=False, order=0)
...
class Binarization:
    ...
    def gray_conversion(self, img_skew_corrected):
        gray_image = cv2.cvtColor(img_skew_corrected, cv2.COLOR_BGR2GRAY)
        return gray_image
...
class NoiseRemoval:
    ...
    def gaussian_blurring (self, binarized_image):
        noise_removed_image = cv2.GaussianBlur(binarized_image, (5,5),0)
